<template>
	<div>
		<div class="title1">酷我音乐</div>
		<div class="title2">温馨提示：【高品质音频请注意流量消耗】,搜索结果数量不生效，每次只展示30条结果</div>
		<div class="all">
			<div class="search">
					...
					<el-row :gutter="24">
						<el-col :span="6">
							<el-form-item label="歌手名/歌曲名：">
								<el-input v-model.trim="dataForm.gm" placeholder="请输入歌手名或歌曲名"></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="6">
							<el-form-item label="音频质量：">
								<el-select v-model="dataForm.br" placeholder="请选择音频质量">
...
export default {
	...
	methods: {
		...
		queryMusic(n = null, index = null) {
			let { gm, br, num } = this.dataForm;
			let url;
			if (n) {
				url = `/eee?type=json&gm=${gm}&br=${br}&num=${num}&n=${n}`;
			} else {
				url = `/eee?type=json&gm=${gm}&br=${br}&num=${num}`;
			}
			axios
				.get(url)
				.then(res => {
					...
					if (n) {
						...
						this.coverImgSrc = res.data.cover;
