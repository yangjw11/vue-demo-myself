<template>
	<div>
		<div class="title1">QQ音乐库(有歌词，不支持分页，只能搜索到20条)</div>
		<div class="all">
			<div class="content">
				<div class="search">
				...
				<el-drawer title="播放列表" :visible.sync="drawer" :direction="direction" :size="drawerWidth">
					<div class="playListArea">
						...
						<div class="optionClass">
							...
							<div class="right">
								<el-button type="primary" @click="refreshAllMusic" :disabled="refreshDisabled">一键更新播放列表所有歌曲链接</el-button>
							</div>
						</div>
						<div class="tableArea">
...
import musicListJSON from '@/views/musicPlayer_qq/info.js';
import musicListJSON_yuexin from '@/views/musicPlayer_qq/info_yuexin.js';
export default {
	data() {
		return {
			musicStr: "周杰伦",
	...
	methods: {
		useDefaultList(type) {
			if(type === 'first') {
				localStorage.setItem("playList_qq", JSON.stringify(musicListJSON));
			} else if(type === 'second') {
				localStorage.setItem("playList_qq", JSON.stringify(musicListJSON_yuexin));
			}
			this.playList = JSON.parse(localStorage.getItem("playList_qq")) || [];
		},
		searchMusic() {
			...
			// `/QQmusic?type=json&msg=${this.musicStrCopy}&quality=${this.musicQuality}&num=${30}&n=${n}`
			let url = `/QQmusic?type=json&msg=${this.musicStrCopy}&key=DragonF51D6A15D46B71E3841B79481C714093`;
			axios
				.get(url)
				.then(res => {
					console.log("res === ", res);
					...
					this.tableList.forEach(item => {
						item.name = item.song_title;
						item.artist = item.song_singer;
		...
		async playAudio(row, index = null, type = null, area = null) {
			...
			this.artist = clickItem.song_singer;
			this.name = clickItem.song_title;
		...
		async getAllMusicInfo(n, type = null) {
			...
			let url = `/QQmusic?type=json&msg=${this.musicStrCopy}&n=${n}&key=DragonF51D6A15D46B71E3841B79481C714093`;
			...
			if (!type) {
				...
				// 歌曲链接
				let musicUrl = resAll[0].data.data.music_url;
				// 歌词
				let musicLyric = resAll[0].data.data.lyric;
				// 歌曲封面
				let musicImg = resAll[0].data.data.cover;
		...
		async addPlayList(row, index) {
			...
			let hasResult = false;
			if (this.playList.length) {
				hasResult = this.playList.some(item => item.music_url === targetItem.music_url || (item.song_singer === targetItem.song_singer && item.song_title === targetItem.song_title));
				if (targetItem.id) {
		...
		saveToLocalStorage() {
			...
			if (playListDeepClone.length) {
				...
				localStorage.setItem("playList_qq", null);
			}
		},
		rowDrop() {
